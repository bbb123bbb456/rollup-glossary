...
A governance structure without a central authority which rewards good behavior and penalizes bad behavior by a set of pre-defined rules which can only be changes by a vote, which typically requires a stake, adding risk to the processto discourage bad actors, amongst the participants.

## Decentralized Domain Name System (DDNS)

A system for registering and resolving domain names on the blockchain, rather than through a centralized domain registrar.
